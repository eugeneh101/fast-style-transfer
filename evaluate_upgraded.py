...
def ffwd(data_in, paths_out, checkpoint_dir, device_t='/gpu:0', batch_size=4):
    ...
    if is_paths:
        ...
        img_shape = X[0].shape
    device_type, device_number = device_t.strip('/').split(':')
    if device_type == 'gpu': # /gpu:0 means use GPU 0; /gpu:1 means use GPU 1; /gpu2: means use GPU2; etc.
        ...
        sess_conf = tf.ConfigProto(intra_op_parallelism_threads=int(device_number), allow_soft_placement=True)
    g = tf.Graph()
    batch_size = min(len(paths_out), batch_size)
    curr_num = 0
    sess_conf.gpu_options.allow_growth = True
    with g.as_default(), tf.Session(config=sess_conf) as sess:
